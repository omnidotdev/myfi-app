  ...
  const handleGenerate = async (params: { asOfDate?: string }) => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_URL}/api/reports/ap-aging?${searchParams.toString()}`,
      );
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-bold text-2xl">AP Aging</h1>
          <p className="text-muted-foreground text-sm">
            Outstanding payables grouped by vendor and aging bucket
          </p>
        </div>
        <BookPicker
          books={books}
      ...
      {!loading && !error && !data && (
        <div className="rounded-lg border border-border bg-card p-8 text-center">
          <p className="text-muted-foreground">
            Select a date and click Generate to view the AP Aging report
          </p>
        </div>
      )}
    </div>
